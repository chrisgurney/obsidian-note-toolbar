...
export class SettingsManager {
	...
	public async duplicateToolbar(toolbar: ToolbarSettings): Promise<string> {
		this.plugin.debug('duplicateToolbar', toolbar);
		let newToolbar = {
			uuid: getUUID(),
			customClasses: "",
		...
		this.plugin.debug('duplicateToolbar: duplicated', newToolbar);
	...
	public getMappedToolbar(frontmatter: FrontMatterCache | undefined, file: TFile): ToolbarSettings | undefined {
		...
		// this.plugin.debug('getMappedToolbar()');

		let matchingToolbar: ToolbarSettings | undefined = undefined;

		// this.plugin.debug('- frontmatter: ', frontmatter);
		...
		if (!matchingToolbar && !ignoreToolbar) {
			...
			for (let index = 0; index < this.plugin.settings.folderMappings.length; index++) {
				...
				// this.plugin.debug('getMatchingToolbar: checking folder mappings: ', filePath, ' startsWith? ', mapping.folder.toLowerCase());
				if (['*'].includes(mapping.folder) || filePath.toLowerCase().startsWith(mapping.folder.toLowerCase())) {
					...
					if (matchingToolbar) {
						// this.plugin.debug('  - matched toolbar:', matchingToolbar);
						break;
					}
				}
			}
	...
	async load(): Promise<void> {
		...
		this.plugin.debug("loadSettings: loaded settings: ", loaded_settings);
		...
		if (loaded_settings && (old_version !== SETTINGS_VERSION)) {
			...
			this.plugin.debug("loadSettings: versions do not match: data.json: ", old_version, " !== latest: ", SETTINGS_VERSION);
			this.plugin.debug("running migrations...");
			...
			if (!old_version) {
				...
				this.plugin.debug("- starting migration: " + old_version + " -> " + new_version);
				// for each: double-check setting to migrate is there
				loaded_settings.toolbars?.forEach((tb: any, index: number) => {
					if (tb.styles) {
						this.plugin.debug("\t- OLD SETTING: " + tb.styles);
						this.plugin.debug("\t\t- SETTING: this.plugin.settings.toolbars[index].defaultStyles: " + this.plugin.settings.toolbars[index].defaultStyles);
						this.plugin.settings.toolbars[index].defaultStyles = tb.styles;
						this.plugin.debug("\t\t- SET: " + this.plugin.settings.toolbars[index].defaultStyles);
						this.plugin.debug("\t\t- SETTING: this.plugin.settings.toolbars[index].mobileStyles = []");
			...
			if (old_version === 20240318.1) {
				...
				loaded_settings.toolbars?.forEach((tb: any, index: number) => {
					tb.items.forEach((item: any, item_index: number) => {
						if (!item?.urlAttr) {
							this.plugin.debug("  - add urlAttr for: ", tb.name, item.label);
			...
			this.plugin.debug("updated settings:", this.plugin.settings);
	...
	async save(): Promise<void> {
		...
		this.plugin.debug("SETTINGS SAVED: " + new Date().getTime());
	}
