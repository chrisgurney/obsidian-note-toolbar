...
export class NoteToolbarSettingTab extends PluginSettingTab {
	...
	// 	rulesContainer.setAttribute('data-active', this.mappingListOpen.toString());
	...
	// 				.setTooltip(t('setting.button-collapse-tooltip'))
	// 				.onClick(async () => {
	// 					let mappingsContainer = containerEl.querySelector('.note-toolbar-setting-rules-container');
	...
	// 						this.mappingListOpen ? heading?.setText(t('setting.rules.name')) : heading?.setText(t('setting.rules.name-with-count', { count: this.plugin.settings.folderMappings.length }));
	...
	// 			let toolbarFolderListItemDiv = this.generateRuleForm(rule);
	 ...
	 */
	// generateRuleForm(rule: ToolbarRule): HTMLDivElement {
