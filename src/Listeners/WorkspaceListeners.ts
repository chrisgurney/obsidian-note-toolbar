...
import { Editor, ItemView, MarkdownFileInfo, MarkdownView, MarkdownViewModeType, Menu, Platform, TAbstractFile, TFile } from "obsidian";
...
export default class WorkspaceListeners {
	...
	register() {
		this.ntb.registerEvent(this.ntb.app.workspace.on('file-open', (file) => this.onFileOpen(file)));
		...
		// add items to menus
		this.ntb.registerEvent(this.ntb.app.workspace.on('file-menu', (menu, file, source) => this.onFileMenu(menu, file, source)));
