...
export default class NoteToolbarPlugin extends Plugin {
	...
	toolbarObserver: IntersectionObserver | undefined;
	activeLeaves: string[] = [];
	...
	async onload() {
		...
		this.registerEvent(this.app.workspace.on('file-open', (event) => { debugLog('FILE-OPEN: ', event) }));
	...
	async storeLeafId(currentView: MarkdownView) {
		// @ts-ignore
		this.activeLeaves.push(currentView?.file?.path + '_' + currentView?.leaf.id);
	}

	haveLeafId(currentView: MarkdownView): boolean {
		// @ts-ignore
		return this.activeLeaves.contains(currentView?.file?.path + '_' + currentView?.leaf.id);
	}

	async removeLeafId(idToRemove: string) {
		this.activeLeaves.remove(idToRemove);
	}
	...
	fileOpenListener = (file: TFile) => {
		...
		if (file != null) {
			...
			// TODO! observer for properties here
	...
	fileOpenListener = (file: TFile) => {
		...
		if (file != null) {
			...
			switch(viewMode) {
				case "preview":
					// let existingPreviewToolbarEl: HTMLElement | null = this.getToolbarEl();
					// debugLog('file-open: (preview) toolbar: ', existingPreviewToolbarEl);
					...
					break;
				case "source":
					let existingSourceToolbarEl: HTMLElement | null = this.getToolbarEl();
					debugLog('file-open: (source) toolbar: ', existingSourceToolbarEl);
	...
	*/
	...
	/**
	 * On leaf changes, check and render toolbar if necessary. 
	 */
	leafChangeListener = (event: any) => {
		...
		if (viewMode === "preview" || viewMode === "source") {
			// debounce seems to be needed here, even with a 0 delay
			// this.app.workspace.onLayoutReady(debounce(() => {
			switch (viewMode) {
				case "preview":
					debugLog("- preview: props el: ", this.getPropsEl());
					// TODO: what's necessary here now?
					let stickyToolbar = currentView?.containerEl.querySelector('.cg-note-toolbar-reading-sticky');
					// let stickyToolbar = currentView?.containerEl.querySelector('.workspace-leaf.mod-active .cg-note-toolbar-reading-sticky');
					if (stickyToolbar) {
						debugLog("leaf-change: unsticking toolbar: ", stickyToolbar);
						stickyToolbar.removeClass('cg-note-toolbar-reading-sticky');
						debugLog("leaf-change: should be unstuck: ", stickyToolbar);
						// put the toolbar back
						let toolbarMarker = currentView?.containerEl.querySelector('#cg-note-toolbar-marker');
						debugLog("leaf-change: found marker? ", toolbarMarker);
						toolbarMarker?.insertAdjacentElement("afterend", stickyToolbar);
					}
					// removing all toolbars on layout change too aggressive, as triggered with plugins like Hover Editor
					let thisToolbarEl: HTMLElement | null = this.getToolbarEl();
					if (thisToolbarEl?.nextElementSibling?.hasClass('inline-title')) {
						debugLog("leaf-change: removing invalid preview toolbar");
						thisToolbarEl.remove();
						thisToolbarEl = null;
					}
					this.renderToolbarForActiveFile();
					break;
				case "source":
					this.renderToolbarForActiveFile();
					let thatToolbarEl: HTMLElement | null = this.getToolbarEl();
					thatToolbarEl?.removeClass('cg-note-toolbar-reading-sticky');
					break;
				default:
					return;
			}
	...
	layoutChangeListener = () => {
		...
		// note: had a 200ms delay around this (debounce) previously for Preview mode only
		// if needed, potentially consider a timer (not 100% sure debounce() is the right move here)
		if (viewMode === "preview" || viewMode === "source") {
			let toolbarEl: HTMLElement | null = this.getToolbarEl();
			switch (viewMode) {
				case "preview":
					debugLog("- preview: props el: ", this.getPropsEl());
					// TODO: what's necessary here now?
					if (toolbarEl?.hasClass('cg-note-toolbar-reading-sticky')) {
						debugLog("layout-change: unsticking toolbar: ", toolbarEl.cloneNode());
						toolbarEl.removeClass('cg-note-toolbar-reading-sticky');
						debugLog("layout-change: should be unstuck: ", toolbarEl.cloneNode());
						// put the toolbar back
						let toolbarMarker = currentView?.containerEl.querySelector('#cg-note-toolbar-marker');
						debugLog("layout-change: found marker? ", toolbarMarker);
						toolbarMarker?.insertAdjacentElement("afterend", toolbarEl);
					}
					// removing all toolbars on layout change too aggressive, as triggered with plugins like Hover Editor
					// let thisToolbarEl: HTMLElement | null = this.getToolbarEl();
					if (toolbarEl?.nextElementSibling?.hasClass('inline-title')) {
						debugLog("layout-change: removing invalid preview toolbar");
						toolbarEl.remove();
						toolbarEl = null;
					}
					// rendering only needed in leaf-change?
					this.renderToolbarForActiveFile();
					break;
				case "source":
					// rendering only needed in leaf-change?
	...
	async checkAndRenderToolbar(file: TFile, frontmatter: FrontMatterCache | undefined): Promise<void> {
		...
		let toolbarRemoved: boolean = this.removeToolbarIfNeeded(matchingToolbar, existingToolbarEl);

		// render the toolbar if we have one, and we don't have an existing toolbar to keep
		debugLog('checkAndRenderToolbar: render? ', matchingToolbar, existingToolbarEl);
		// if (matchingToolbar && (existingToolbarEl === null)) {
		if (matchingToolbar && toolbarRemoved) {
			debugLog("checkAndRenderToolbar: YES rendering: ", matchingToolbar, " for file: ", file);
			this.renderToolbarFromSettings(matchingToolbar);
		}
		else if (existingToolbarEl !== null) {
			debugLog('checkAndRenderToolbar: NO, move?');
			// check if the toolbar is next to the marker and move it there if it's not
			let currentView = this.app.workspace.getActiveViewOfType(MarkdownView);
			let toolbarMarker = currentView?.containerEl.querySelector('#cg-note-toolbar-marker') as HTMLElement;
			if (toolbarMarker.nextElementSibling !== existingToolbarEl) {
				toolbarMarker.insertAdjacentElement("afterend", existingToolbarEl);
				debugLog('checkAndRenderToolbar: MOVED after marker');
			}
		}
	...
	async renderToolbarFromSettings(toolbar: ToolbarSettings): Promise<void> {
		...
		let toolbarMarker = currentView?.containerEl.querySelector('#cg-note-toolbar-marker') as HTMLElement;
		if (!toolbarMarker) {
			// if marker doesn't exist yet, inject it between the properties and content divs
			// TODO: can we check for preview mode + intersection here, so it can be sticky right away?
			...
			toolbarMarker = document.createElement("div");
			toolbarMarker.id = "cg-note-toolbar-marker";
			propsContainer?.insertAdjacentElement("afterend", toolbarMarker);
			debugLog('-> marker position: props');
		}
		// insert the toolbar after the marker
		toolbarMarker.insertAdjacentElement("afterend", embedBlock);

		// FIXME: need to check for these styles in Observer, as they could be changed at any time
		// workaround for sticky not working in Reading mode: observe the toolbar's position and move it
		if ([...toolbar.defaultStyles, ...toolbar.mobileStyles].find((style) => ['sticky', 'mstky'].includes(style))) {
			// FIXME: can instantiate in onload() or in one of the event handlers?
			// instantiate the observer, if it's not already
			if (this.toolbarObserver === undefined) {
				this.toolbarObserver = new IntersectionObserver(this.toolbarVisibilityHandler, {
					root: this.app.workspace.getActiveViewOfType(MarkdownView)?.contentEl,
					rootMargin: '1px',
					threshold: 0
				});
				debugLog('- observer created 👀');
			}
			// FIXME: move to where the file is first opened and observe the properties
			// check if the marker is already being observed, and if not, observe it
			// const entries = this.toolbarObserver?.takeRecords().filter(entry => entry.target === toolbarMarker);
			// entries?.length === 0 ? this.toolbarObserver?.observe(toolbarMarker) : undefined;
			// debugLog('- observing marker 👀');

			let propsEl = document.querySelector('.workspace-leaf.mod-active .metadata-container');
			propsEl ? this.toolbarObserver?.observe(propsEl) : undefined;
			debugLog('- observing props 👀');
		}
	...
	private moveToolbar() {
		...
		// assumes: there should be a toolbar and a marker on this note
		let currentView = this.app.workspace.getActiveViewOfType(MarkdownView);
		let existingToolbarEl: HTMLElement | null = this.getToolbarEl();
		let toolbarMarker = currentView?.containerEl.querySelector('#cg-note-toolbar-marker');
		debugLog('moveToolbar()');
		debugLog('- have a toolbar?', existingToolbarEl !== null, 'have a marker?', toolbarMarker !== null);

		if (existingToolbarEl && toolbarMarker) {
			...
			if (viewMode === 'source') {
				...
				// marker and toolbar should be below props and above content always
				let propsEl = this.getPropsEl();
				debugLog('- are there props? ', propsEl !== null);
				// should be marker
				let propsElSibling = propsEl?.nextElementSibling;
				let isPropsSiblingMarker = propsElSibling?.id === 'cg-note-toolbar-marker';
				debugLog('- is props sibling the marker? ', isPropsSiblingMarker, propsElSibling);

				if (toolbarMarker && !isPropsSiblingMarker) {
					propsEl?.insertAdjacentElement("afterend", toolbarMarker);
					debugLog('-> marker moved to props: ', toolbarMarker);
					existingToolbarEl ? toolbarMarker.insertAdjacentElement("afterend", existingToolbarEl) : undefined;
					debugLog('-> toolbar moved to marker: ', existingToolbarEl);
				}

				// // toolbar should be next to marker
				// let isMarkerSiblingToolbar: boolean = ((toolbarMarker?.nextElementSibling ?? false) && toolbarMarker?.nextElementSibling?.hasClass('cg-note-toolbar-container')) ?? false;
				// debugLog('- is marker sibling the toolbar? ', isMarkerSiblingToolbar, propsElSibling?.nextElementSibling);

				// if (!isMarkerSiblingToolbar) {
				// 	this.renderToolbarForActiveFile();
				// }
			...
			// in reading mode
				...
				// FIXME: is this what we do to know if the toolbar's visible?
				if (this.toolbarObserver) {
					let entries = this.toolbarObserver?.takeRecords().filter(entry => entry.target === toolbarMarker);
					// if (entries.length)
					debugLog('- observer entries: ', entries);
				}
				...
				// 	this.renderToolbarForActiveFile();
				// }
				// (different parent in Reading mode?)
				// might want to check: does not have a sibling that's the note title
				// might this note not have props?
	...
	/**
	 * Implements sticky toolbars for Preview mode, specifically:
	 * Moves the active toolbar to below the editor's header, and vice-versa.
	 * @param entries Intersection changes for observed elements (in this case the toolbar).
	 */
	toolbarVisibilityHandler: IntersectionObserverCallback = (entries) => {

		// TODO: does this actually correspond with current state of the entries the observer sees?
		let currentView = this.app.workspace.getActiveViewOfType(MarkdownView);
		let viewMode = currentView?.getMode();

		// TODO: do I need to loop over these? can I just use the first and ignore the rest?
		entries.forEach((entry, index) => {
			debugLog("toolbarObserver: observed: ", index, entry.target);
			if (entry.isIntersecting) {
				debugLog("toolbarObserver: IN VIEW: ", entry.target);
				let toolbarMarker = currentView?.containerEl.querySelector('#cg-note-toolbar-marker');
				let readingStickyToolbar = currentView?.containerEl.querySelector('.cg-note-toolbar-reading-sticky');
				if (readingStickyToolbar) {
					debugLog('- toolbarVisibilityHandler: existing sticky toolbar: ', readingStickyToolbar);
					readingStickyToolbar.removeClass('cg-note-toolbar-reading-sticky');
					// put the toolbar back
					toolbarMarker?.insertAdjacentElement("afterend", readingStickyToolbar);
					debugLog("-> toolbarVisibilityHandler: put toolbar back");
				}
				else {
					// FIXME: this may be too much to add it over and over again here
					let toolbar: HTMLElement | null = this.getToolbarEl();
					if (toolbar) {
						toolbarMarker?.insertAdjacentElement("afterend", toolbar);
					}
				}
			}
			else {
				debugLog("toolbarObserver: NOT IN VIEW: ", entry.target);
				if (currentView && viewMode === "preview") {
					let readingStickyToolbar = currentView?.containerEl.querySelector('.cg-note-toolbar-reading-sticky');
					if (!readingStickyToolbar) {
						// TODO: can we just get the toolbar at the top of the function and use it instead of querying for the bar again?
						let toolbar: HTMLElement | null = this.getToolbarEl();
						if (toolbar) {
							toolbar.addClass('cg-note-toolbar-reading-sticky');
							currentView.containerEl.find(".view-header").insertAdjacentElement("afterend", toolbar);
							debugLog("toolbarObserver: TOOLBAR STUCK: ", toolbar);
						}
					}
				}
			}
	...
	private getPropsEl(): HTMLElement | null {
		...
		let propertiesContainers = currentView?.contentEl.querySelectorAll('.metadata-container');
		if (propertiesContainers && propertiesContainers?.length > 1) {
			debugLog("getPropsEl: more than one metadata-container!");
		}
		// let propertiesContainer = currentView?.contentEl.querySelector('.metadata-container') as HTMLElement;
		// let propertiesContainer = this.app.workspace.containerEl.querySelector('.cm-editor > .metadata-container');
		// let propertiesContainer = currentView?.contentEl.querySelector('.workspace-leaf.mod-active .markdown-' + currentView?.getMode() + '-view .metadata-container') as HTMLElement;
		...
		// let propertiesContainer = currentView?.containerEl.querySelector('.workspace-leaf.mod-active .metadata-container') as HTMLElement;
		// let propertiesContainer = document.querySelector('.workspace-leaf.mod-active .metadata-container') as HTMLElement;
	...
	private getToolbarEl(): HTMLElement | null {
		...
		let existingToolbarEl = currentView?.containerEl.querySelector('.workspace-leaf.mod-active .cg-note-toolbar-container') as HTMLElement;
	 ...
	 */
	private removeToolbarIfNeeded(correctToolbar: ToolbarSettings | undefined, existingToolbarEl: HTMLElement | null = null): boolean {
