...
export default class ToolbarElementHelper {
	...
	getActiveItemId(): string | null {
		return this.plugin.app.loadLocalStorage(LocalVar.ActiveItem);
	}
    ...
    getAllToolbarEl(view?: ItemView): NodeListOf<HTMLElement> {
        let toolbarViewEl = view ? view.containerEl : this.plugin.app.workspace.getActiveViewOfType(ItemView)?.containerEl as HTMLElement;
    ...
    getPropsEl(view?: MarkdownView): HTMLElement | null {
        const currentView = view ?? this.plugin.app.workspace.getActiveViewOfType(MarkdownView);
        ...
        if (this.plugin.hasPlugin['make-md'] && propertiesContainer?.closest('.mk-frame-edit')) {
            return null;
        }
    ...
    getOutputEl(calloutMeta: string): HTMLElement | null {
        const currentViewEl = this.plugin.app.workspace.getActiveViewOfType(MarkdownView)?.leaf.containerEl as HTMLElement | null;
    ...
    getToolbarEl(view?: ItemView, isTextToolbar: boolean = false): HTMLElement | null {
        if (isTextToolbar) {
            ...
            const toolbarView = view ? view : this.plugin.app.workspace.getActiveViewOfType(ItemView);
