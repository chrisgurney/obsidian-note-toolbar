...
- Setting to display toolbars when using the keyboard to select text (under **Toolbars within the app → Display for keyboard selections**), which is _enabled_ by default. Disabling this setting _should_ fix [#467](https://github.com/chrisgurney/obsidian-note-toolbar/issues/467). _Thank you @AlbusGuo!_
...
- `v1.27.22` Added **Z-index** CSS variables and Style Settings for floating buttons, top/bottom toolbars, and floating toolbars. This can be used to fix toolbar overlapping issues with plugins (such as [Dynamic Outline](https://github.com/theopavlove/obsidian-dynamic-outline)) not directly supported by Note Toolbar. _Thanks u/arsenalbilbao [on Reddit](https://www.reddit.com/r/ObsidianMD/comments/1p6kqoe/comment/o0oyrko/?utm_source=share&utm_medium=web3x&utm_name=web3xcss&utm_term=1&utm_content=share_button)._
- `v1.27.19` Phones: Style Setting: Added setting for horizontal padding around Top-positioned toolbars.
- `v1.27.16` Phones: Improved the contrast of `glass` style items.
- `v1.27.06` Phones: The scroll bar is now hidden when using the don't wrap items style.
- `v1.27.05` For disabled toolbar items, "Item not available" is now shown in parentheses if there's an existing tooltip (e.g., "Toggle bold (Item not available)"). _Thanks u/arsenalbilbao on Reddit._
- `v1.27.05` Settings: Removed the settings tab icon for now, until more plugin developers add their own icons. _Thanks to anonymous user via the feedback form._
- `v1.27.04` Menu items: Command items that aren't available are now shown as disabled (versus not being displayed at all).
...
- Copy a toolbar's developer ID in Note Toolbar's main settings, using **More options (...) → Copy developer ID**, located next to each toolbar. Use this as another method to uniquely style toolbars, or use them via the API without worrying if its name might change.
...
- `v1.27.25` Phones: Fix for bottom toolbar positioning issues. [#479](https://github.com/chrisgurney/obsidian-note-toolbar/issues/479)
- `v1.27.23` Plugin installation error on iOS 15, due to the use of a regex lookbehind (used by the callout import code). [#451](https://github.com/chrisgurney/obsidian-note-toolbar/issues/451)
- `v1.27.22` Cupertino + Baseline themes: Style Settings for background colors were not applying to the toolbar or buttons. _Thanks @jebeaudoin for [reporting this issue](https://github.com/chrisgurney/obsidian-note-toolbar/discussions/409#discussioncomment-15550743)._
- `v1.27.18` Phones: Bottom-positioned toolbars with the "don't wrap items" style applied are now scrollable.
- `v1.27.07` Phones: Don't allow swipes to open sidebars in `don't wrap items` toolbars.
- `v1.27.06` Phones: Floating buttons no longer show over top of the right sidebar.
- `v1.27.06` Phones: On startup, if the Backlinks built-in plugin's panel was previously open, it became the active view and no toolbar was rendered.
- `v1.27.06` Phones: Using items in the toolbar Bottom position was opening the Navbar. [#477](https://github.com/chrisgurney/obsidian-note-toolbar/issues/477) _Thank you for reporting this @same774_
- `v1.27.06` Phones: A Bottom-positioned toolbar was moving up even if the Navbar was fully hidden.
- `v1.27.06` Phones: Obsidian's Navbar position was not being re-adjusted after navigating away from a note with a floating button.
