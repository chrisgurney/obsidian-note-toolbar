...
Select a toolbar to appear in Web viewer tabs, under **Toolbars within the app → Web viewer**.

- The [Web viewer core plugin ↗](https://help.obsidian.md/plugins/web-viewer) must be enabled to see this setting.
...
- `v1.29.05` Help section: Moved the _What’s new_ button to a link, so the section now takes up less room on smaller screens.
- `v1.29.05` Some toolbar list options weren’t applicable if the toolbar was empty, so they’re now hidden.
- `v1.29.05` Gallery: The toolbar suggester is skipped on adding an item, if there aren't any existing toolbars.

### Other improvements

- `v1.29.14` Gallery: Added navigation for daily notes with the [Periodic Notes ↗](https://github.com/liamcain/obsidian-periodic-notes) plugin.
...
### `ntb.suggester()` icon, and collapsed style
...
- Set `collapsed: true` to only show the input field of the suggester, until input is entered. Defaults to `false`.
  - Note that the UX is not yet finalized on phones.
