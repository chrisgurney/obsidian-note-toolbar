...
- `v1.29.04` All new toolbars now have a default name of `Toolbar`, rather than leaving the field empty.
- `v1.29.04` A warning is shown if the toolbar being deleted is an app toolbar (such as the _Text selection_ toolbar).
- `v1.29.04` Help section: Moved the _What’s new_ button to a link, so the section now takes up less room on smaller screens.
- `v1.29.04` Some toolbar list options weren’t applicable if the toolbar was empty, so they’re now hidden.
- `v1.29.04` Gallery: The toolbar suggester is skipped on adding an item, if there aren't any existing toolbars.
