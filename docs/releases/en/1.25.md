...
- Under **Advanced settings**, script items (i.e., Dataview, JavaScript, JS Engine, Templater) have a **Focus in editor** option that defaults to `true`: After scripts are executed, the focus is placed in the editor; if set to `false`, it won't be.
- For script items, the **Output callout ID** option has been moved into the **Advanced settings** section.
