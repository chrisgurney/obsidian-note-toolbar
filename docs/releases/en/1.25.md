...
- Under **Advanced script settings**, script items (i.e., Dataview, JavaScript, JS Engine, Templater) have a **Focus in editor** option that defaults to `true`: After scripts are executed, the focus is placed in the editor; if set to `false`, it won't be.
