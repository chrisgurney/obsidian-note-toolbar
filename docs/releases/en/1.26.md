...
- A new `{{selection}}` [variable](https://github.com/chrisgurney/obsidian-note-toolbar/wiki/Variables) (for labels, tooltips, and URIs) is populated with the currently selected text (via the new `ntb.getSelection()` API), or with the word at the cursor position.
  - Encode it via `{{encode:selection}}` for inclusion in URIs.
- Note: Toolbars with breaks have a gap on the right side (equivalent to the size of the wrapped items).
...
- Settings: Clicking on the headers of collapsible settings sections now also toggles the visibility of that section.
- Settings: Note Toolbar's main settings has a new section that collects the options for toolbars that can accessed across the app (i.e., outside of notes).
- The Item Suggester now has a more prominent _Browse Gallery..._ link.
