...
Encode it via `{{encode:selection}}` for inclusion in URIs. For example, search for the current selection with a URI item with:

```ts
obsidian://search?query={{encode:selection}}
```
