...
Encode it via `{{encode:selection}}` for inclusion in URIs. For example, search for the current selection with a [URI item](https://github.com/chrisgurney/obsidian-note-toolbar/wiki/URI-items) with this URI: `obsidian://search?query={{encode:selection}}`
