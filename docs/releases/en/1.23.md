...
URIs that are external links can now be opened in new tabs, split views, or in a new window if it is an external link (web URL). Modifier keys are now also supported.

- Note Toolbar will open the link in [Obsidian's Web Viewer ↗](https://help.obsidian.md/plugins/web-viewer), if the core plugin is enabled; otherwise, the link will open in the system's default browser.
